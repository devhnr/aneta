...
class Front_productcontroller extends Controller
{
    ...
    public function product_listing(Request $request, $groupurl='',$cat_url='',$subcat_url=''){
        ...
        if ($groupurl != '') {
			...
			$sizesProducts = $sizesProducts->where('p.group_id', '=', $groupdata->id);
        ...
        if ($subcat_url != '') {
            ...
            $sizesProducts = $sizesProducts->where('p.subcat_id', '=', $subcat_data->id);
       ...
       if($colorIds !== null && $request->get('colour') !== null){
		 ...
		 $sizeIds  =  $request->get('size');
       if($sizeIds !== null && $request->get('size') !== null){

            $query = $query->whereIn('pa.size_id', $sizeIds);
            $data['sizeIds'] = implode(",",$sizeIds);
